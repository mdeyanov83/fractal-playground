...
// GLOBAL VARIABLES
let gl, glJulia;
let mainCanvas, juliaCanvas;
let mainShaderProgram, juliaShaderProgram;
let resolutionLoc, centerLoc, zoomLoc, iterationsLoc;
...
export function renderSavedView (x, y, zoom, iter) {
    ...
    zoomMain = zoom;
    maxIterations = iter;
    itersInput.value = iter;
    itersSlider.value = iter;
    updateDetails(centerX, centerY, zoomMain, "center");
    renderMain();
...
// This computation is used when dynamically tracking the cursor inside the Main canvas element.
...
// Create all event listeners after DOM has been constructed
document.addEventListener("DOMContentLoaded", () => {
    ...
    mainCanvas.addEventListener("wheel", (e) => {
        ...
        let pixelOffsetX = (mouseX - 0.5) * (4.0 / zoomMain) * aspectRatio;
        let pixelOffsetY = (0.5 - mouseY) * (4.0 / zoomMain);
        ...
        if (zoomMain*zoomFactor <= MAX_ZOOM) {
            zoomMain *= zoomFactor;
        } else {
            zoomMain = MAX_ZOOM;
        }
        ...
        pixelOffsetX = (mouseX - 0.5) * (4.0 / zoomMain) * aspectRatio;
        pixelOffsetY = (0.5 - mouseY) * (4.0 / zoomMain);
    ...
    mainCanvas.addEventListener("mousedown", (e) => {
        ...
        // Rasterizer - canvas width and height are reduced for smoother pan/zoom
    ...
    mainCanvas.addEventListener("mousemove", (e) => {
        if (isDragging) {
            let aspectRatio = mainCanvas.width / mainCanvas.height;
            let dx = (e.clientX - lastX) / mainCanvas.width * (4.0 / zoomMain) * aspectRatio * dpr / 2;
            let dy = (lastY - e.clientY) / mainCanvas.height * (4.0 / zoomMain) * dpr / 2;
            centerX -= dx;
            centerY -= dy;
            lastX = e.clientX;
            lastY = e.clientY;
    ...
    mainCanvas.addEventListener("mousemove", (e) => {
        ...
        let mouseXClip = (mouseX - 0.5) * (4.0 / zoomMain) * aspectRatio + centerX;
        let mouseYClip = (0.5 - mouseY) * (4.0 / zoomMain) + centerY;

        updateDetails(mouseXClip, mouseYClip, zoomMain, "cursor");
    ...
    // Event listener Pinch-to-Zoom (Mobile)



    // Event listener Drag-to-Pan (Mobile)
...
function main() {
    ...
    gl = getContext(mainCanvas);
    glJulia = getContext(juliaCanvas);
    ...
    // Create program
    mainShaderProgram = createProgram(gl, vertexShaderSourceCode, fragmentShaderSourceCodeMand)
    juliaShaderProgram = createProgram(glJulia, vertexShaderSourceCode, fragmentShaderSourceCodeJulia)

    // Get Attributes locations
    vertexPositionAttribLocation = gl.getAttribLocation(mainShaderProgram, "vertexPosition");
    vertexPositionAttribLocationJulia = glJulia.getAttribLocation(juliaShaderProgram, "vertexPosition");
    // Error check
    ...
    // Get Main Uniform locations
    resolutionLoc = gl.getUniformLocation(mainShaderProgram, "u_resolution");
    centerLoc = gl.getUniformLocation(mainShaderProgram, "u_center");
    zoomLoc = gl.getUniformLocation(mainShaderProgram, "u_zoom");
    iterationsLoc = gl.getUniformLocation(mainShaderProgram, "u_iterations");
    // Get Julia set uniform locations
    resolutionLocJulia = glJulia.getUniformLocation(juliaShaderProgram, "u_resolution");
    centerLocJulia = glJulia.getUniformLocation(juliaShaderProgram, "u_center");
    zoomLocJulia = glJulia.getUniformLocation(juliaShaderProgram, "u_zoom");
    iterationsLocJulia = glJulia.getUniformLocation(juliaShaderProgram, "u_iterations");
    ...
    // Not necessary in this case, important in 3d rendering
